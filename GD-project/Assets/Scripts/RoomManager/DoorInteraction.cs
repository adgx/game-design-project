...
namespace RoomManager // Or your game's specific namespace
{
	public class DoorInteraction : MonoBehaviour {
		...
		void Start() {
			...
			if(parentRoom == null)
				Debug.LogError("DoorInteraction: Parent Room component not found! Ensure this script is a child of a Room GameObject.", this);
		...
		private void InferDirectionFromName() {
			...
			if(myNameLower.Contains("north") || myNameLower.Contains("top"))
				...
				Debug.LogWarning($"DoorInteraction on '{gameObject.name}': Could not infer 'Leads To World Direction' from name. Please set it manually in Inspector for reliable behavior.", this);
		}

		void Update() {
			// Basic validity checks
			if(roomManager == null || parentRoom == null || roomManager.currentPlayer == null || !roomManager.playerHasSpawned)
				return;
			if(leadsToWorldDirection == Vector3Int.zero)
				return; // This door isn't configured to lead anywhere
